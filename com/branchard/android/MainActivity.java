...
public class MainActivity extends Activity {
	...
	public void setHead(String userId) {
		...
		if (file.exists()) {
			...
			head.setImageResource(R.drawable.ad1);
		}
	}
	...
	public void putHeadToCard(){
		...
		for(int i=0;i<10;i++){
			File f = new File("/sdcard/head/", head[i]);//创建 了一个代表这个路径的file对象
			if (f.exists()) {//先判断存不存在
				f.delete();
			}
			else{
			...
			try {
				FileOutputStream out = new FileOutputStream(f);//创建输出目的地
				bm[i].compress(Bitmap.CompressFormat.JPEG, 100, out);//采样率法压缩图片
				out.flush();
				out.close();
